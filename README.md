...
32x32 RGB images that look like CIFAR-10 photos.
...
- A short theory recap of the GAN minimax game and the main DCGAN
...
- Loss curves, a final 8x8 grid of generated images, and saved model
...
For the all-classes run on a GPU, bump `EPOCHS` up to 80-150 to get sharper
samples.

## Outputs

- `samples/epoch_XXX.png`: 8x8 grid of fakes from a fixed noise vector,
  written every epoch so you can flip through them as a stop-motion of training.
- `samples/final.png`: final 8x8 grid from a fresh noise vector.
